...
import { packagingTypes } from "./schema";
...
async function seed() {
  ...
  await db.insert(products).values([
    {
      ...
      category1: "Food & Beverage",
      category2: "Packaging",
      material1: "Food Ingredients",
      material2: "Paper",
      weight: "1.00",
      sellerId,
    },
    ...
    },
    {
      ...
      supportsEcoPackaging: true,
      deliveryType: ["standard"],
      sector: "Home durables, textiles, & equipment",
      category1: "Home Essentials",
  ...
  const reviewRows = allProducts.flatMap((p, idx) => {
    const u1 = allUsers[idx % allUsers.length].id;
    const u2 = allUsers[(idx + 1) % allUsers.length].id;
    return [
      {
        productId: p.id,
        userId: u1,
